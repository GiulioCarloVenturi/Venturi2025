function eq=SS_eq_noCBDC_cash(func_var,Nb,y)
%% Parameters
...
z_func = @(x) interp1(y(:,1),y(:,2),min(x,Z_bar));
%% Loan Supply and Demand at Lowest R_L
Dlow = fzero(@(x) xi_func(r_reserves).*(psi_func(x)./((1-mu_db).*(1-dpsi_func(x).*x/Nb)) )-1,[0,10]);
LS_low = (1-rq_ratio)*Dlow*psi_func(Dlow);
LD_high = df_inv(r_reserves);
%% Loan Supply and Demand at Highest R_L
Dbar = fzero(@(x) xi_func(rho_bar).*(psi_func(x)./((1-mu_db).*(1-dpsi_func(x).*x/Nb)) )-1,[0,D_bar]);
LS_bar = (1-rq_ratio)*Dbar*psi_func(Dbar);
LD_low = df_inv(rho_bar);
%% Loan Supply and Demand at R_L
rho_func = @(x) df(x)-1; 
eq_D_func = @(x) xi_func(rho_func(psi_func(x).*x*(1-rq_ratio))).*(psi_func(x)./((1-mu_db).*(1-dpsi_func(x).*x/Nb)) )-1;
