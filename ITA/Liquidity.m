...
ID_deposit_nofee = find(liquidity_nofee(:,2) > liquidity_nofee(1,2),1,'first');
ID_deposit_fee = find(liquidity_fee(:,2) > liquidity_fee(1,2),1,'first');
...
xt_d = [icbdc_grid(ID_deposit_nofee) icbdc_grid(ID_max_nofee) icbdc_grid(ID_min_nofee)...
    icbdc_grid(ID_deposit_fee) icbdc_grid(ID_max_fee)   icbdc_grid(ID_min_fee) ]*100;
yt_d = [liquidity_nofee(ID_deposit_nofee,2) liquidity_nofee(ID_max_nofee,2) liquidity_nofee(ID_min_nofee,2)...
      liquidity_fee(ID_deposit_fee,2) liquidity_fee(ID_max_fee,2) liquidity_fee(ID_min_fee,2)];
str_d = {'A','B','C','D','E','F'};
f=figure;
hold on
plot(icbdc_grid*100,liquidity_nofee(:,2),'k',LineWidth=3)
plot(icbdc_grid*100,liquidity_fee(:,2),'r',LineWidth=3)
plot(icbdc_grid(ID_deposit_nofee)*100,liquidity_nofee(ID_deposit_nofee,2),'k.','MarkerSize',30)
...
plot(icbdc_grid(ID_deposit_fee)*100,liquidity_fee(ID_deposit_fee,2),'k.','MarkerSize',30)
...
text(xt_d,yt_d+0.005,str_d,'Interpreter','latex','FontSize',15)
xlim([0,i_m*100])
...
print('ITA_liquidity_deposits','-dpdf','-r200','-fillpage')

xt_cb = [icbdc_grid(ID_deposit_nofee) icbdc_grid(ID_deposit_fee) ...
    icbdc_grid(ID_cbdc_nofee) icbdc_grid(ID_cbdc_fee)]*100;
yt_cb = [0 0 0 0]; 
str_cb = {'A','D','G','H'};
f=figure;
hold on
plot(icbdc_grid*100,liquidity_nofee(:,1),'k',LineWidth=3)
plot(icbdc_grid*100,liquidity_fee(:,1),'r',LineWidth=3)
plot(icbdc_grid(ID_deposit_nofee)*100,0,'k.','MarkerSize',30)
plot(icbdc_grid(ID_deposit_fee)*100,0,'k.','MarkerSize',30)
plot(icbdc_grid(ID_cbdc_nofee)*100,0,'k.','MarkerSize',30)
plot(icbdc_grid(ID_cbdc_fee)*100,0,'k.','MarkerSize',30)
text(xt_cb,yt_cb+0.01,str_cb,'Interpreter','latex','FontSize',15)
xlim([0,i_m*100])
xlabel('$i_{CBDC}$', 'interpreter','latex')
xtickformat("percentage");
ax=gca;
...
set(f,'PaperType','A4');
set(f,'PaperOrientation','landscape');
print('ITA_liquidity_cbdc','-dpdf','-r200','-fillpage')
figure;
hold on
plot(icbdc_grid,deposit_rate_nofee,'k')
plot(icbdc_grid,deposit_rate_fee,'r')
...
rq_ratio = 0.01; 
opt = 1;
fees = [0.0184,0.0123,0.0035,0.0095]; % Cash, card, merchant fee, debit
mu_c = fees(1);
mu_d = fees(2)+fees(3);
mu_db = fees(3);
mu_cb = fees(4);
