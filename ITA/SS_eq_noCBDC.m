...
%Dlow = fzero(@(x) (1-mu_d)*xi_func(rho_low).*(psi_func(x).^2./(psi_func(x)-dpsi_func(x).*x/Nb) )-1,[0,10]);
...
%Dbar = fzero(@(x) (1-mu_d)*xi_func(rho_bar).*(psi_func(x).^2./(psi_func(x)-dpsi_func(x).*x/Nb) )-1,[0,D_bar-1e-3]);
