...
load parameters_ITA_final_f.mat;

sigma=parameters_ITA_final_f(1);
B=parameters_ITA_final_f(2);
Nb = parameters_ITA_final_f(3);
alpha1=parameters_ITA_final_f(4);
alpha2=parameters_ITA_final_f(5);
alpha3=parameters_ITA_final_f(6);
beta=parameters_ITA_final_f(7);
cost=parameters_ITA_final_f(8);
eta=parameters_ITA_final_f(9);                                   
A=parameters_ITA_final_f(10); 
theta=parameters_ITA_final_f(11);
epsilon=0.001;
rq_ratio = 0.01; 
opt = 1;
fees = [0.0184,0.0123,0.0035,0.0095]; % Cash, card, merchant fee
...
%%
fee_cb = 0.001:0.0005:0.02;
for n = 1:size(fee_cb,2)
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
disp(['CBDC cost ' num2str(fee_cb(n)*100) '%']);
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
mu_cb = fee_cb(n);
...
Output_CBDC(:,n) = alpha1*y_func(min((1-mu_c)*cash(:,n),p_star_c))+...
    ...
    alpha3*y_func(min((1-mu_c)*cash(:,n)+(1-mu_d)*deposit(:,n)+(1-mu_cb)*cbdc(:,n),p_star))+...
...
psi_D(:,n) = deposit(:,n)./(1+deposit_rate(:,n));
%% Summarize Results
...
[~,id_max] = min(abs(fee_cb-fees(end)));
[~,id_opt] = max(output_max);
f=figure;
set(f,'PaperType','A4');
set(f,'PaperOrientation','landscape');
hold on
plot(fee_cb*100,i_min1*100,'--k','LineWidth',2)
plot(fee_cb*100,i_max*100,'k','LineWidth',2)
plot(fee_cb*100,i_max1*100,'--k','LineWidth',2)
plot(fees(end)*100,i_max(id_max)*100,'k.','MarkerSize',30)
plot(fee_cb(id_opt)*100,i_max(id_opt)*100,'k*','MarkerSize',15)
ytickformat("percentage")
xtickformat("percentage")
xlabel('CBDC cost','Interpreter','latex','FontSize',15)
title('Optimal CBDC Rate','Interpreter','latex','FontSize',20)
print('ITA_costcomp_rate','-dpdf','-r200','-fillpage')

f=figure;
set(f,'PaperType','A4');
set(f,'PaperOrientation','landscape');
hold on
plot(fee_cb*100,(loan_max-1)*100,'k','LineWidth',2)
plot(fees(end)*100,(loan_max(id_max)-1)*100,'k.','MarkerSize',30)
plot(fee_cb(id_opt)*100,(loan_max(id_opt)-1)*100,'k*','MarkerSize',15)
ylim([0.0 2.2])
ytickformat("percentage")
xtickformat("percentage")
title('Loan Increase','Interpreter','latex','FontSize',20)
xlabel('CBDC cost','Interpreter','latex','FontSize',15)
print('ITA_costcomp_loan','-dpdf','-r200','-fillpage')

f=figure;
set(f,'PaperType','A4');
set(f,'PaperOrientation','landscape');
hold on
plot(fee_cb*100,output_max,'k','LineWidth',2)
plot(fees(end)*100,output_max(id_max),'k.','MarkerSize',30)
plot(fee_cb(id_opt)*100,output_max(id_opt),'k*','MarkerSize',15)
ylim([0.0 0.35])
ytickformat("percentage")
xtickformat("percentage")
title('Output Increase','Interpreter','latex','FontSize',20)
xlabel('CBDC cost','Interpreter','latex','FontSize',15)
print('ITA_costcomp_output','-dpdf','-r200','-fillpage')

disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
disp('Optimal CBDC Cost');
disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
disp(['Cost: ' num2str(fee_cb(id_opt)*100) '%'])
disp(['Loan Increase: ' num2str((loan_max(id_opt)-1)*100) '%'])
disp(['Output Increase: ' num2str(output_max(id_opt)) '%'])
disp(['Optimal Rate: ' num2str(i_max(id_opt)*100) '%'])
disp(['Optimal vs Preferred: ' num2str((output_max(id_opt)/output_max(id_max)-1)*100) '%'])