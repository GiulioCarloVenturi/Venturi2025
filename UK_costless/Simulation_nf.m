...
load parameters_UK_final_nf.mat;

sigma=parameters_UK_final_nf(1);
B=parameters_UK_final_nf(2);
Nb = parameters_UK_final_nf(3);
alpha1=parameters_UK_final_nf(4);
alpha2=parameters_UK_final_nf(5);
alpha3=parameters_UK_final_nf(6);
beta=parameters_UK_final_nf(7);
eta=parameters_UK_final_nf(9);                                   
A=parameters_UK_final_nf(10); 
theta=parameters_UK_final_nf(11);
epsilon=0.001;
opt = 1;
left = find(year(Data_TT.Dates) == 2000,1,"first");
right = find(year(Data_TT.Dates) < 2013,1,"last"); %%
i_reserve = mean(Data_TT.BankRate(left:right))/100; 
pi_m = mean(Data_TT.Inflation(left:right));
pi_cbdc = pi_m;
cost=parameters_UK_final_nf(8);
%% Fees
mu_d = 0.00;
mu_c = 0.00;
mu_db = 0.00;
