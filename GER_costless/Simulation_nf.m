...
load parameters_GER_final_nf.mat;

year_begin = max(2000,min(year(Data_TT.Dates)));
year_end = 2012;
sigma=parameters_GER_final_nf(1);
B=parameters_GER_final_nf(2);
Nb = parameters_GER_final_nf(3);
alpha1=parameters_GER_final_nf(4);
alpha2=parameters_GER_final_nf(5);
alpha3=parameters_GER_final_nf(6);
beta=parameters_GER_final_nf(7);
eta=parameters_GER_final_nf(9);                                   
A=parameters_GER_final_nf(10); 
theta=parameters_GER_final_nf(11);
epsilon=0.001;
...
left = find(year(ECB_rate.Dates) == year_begin,1,"first");
...
r = 1/beta - 1;
%% Inflation Rate
left = find(year(Data_TT.Dates) == year_begin);
right = find(year(Data_TT.Dates) == year_end);
pi_m = (prod(1+Data_TT.Inflation(left:right)/100))^(1/right) - 1; %% Updated, original 0.01515
pi_cbdc = pi_m;
cost = parameters_GER_final_nf(8);
...
%% Save Results
save simulation_GER_nf.mat eq_cbdc icbdc_grid output_index i_m pi_m psi_D...
