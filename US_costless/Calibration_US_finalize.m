...
i_l = mean(Data_TT.LoanRate(left:right));
i_D = mean(Data_TT.DepositRate(left:right));
alpha1 = 0.0458; 
alpha2 = 0.2644; 
alpha3 = 0.6898; 
epsilon = 0.001; 
beta = 0.99;
rq_ratio = 0.02; 
i_reserves = 0.00; %mean(Data_TT.MRRRate(left:right))/100; 
r_reserves = (1+i_reserves)/(1+pi_m)-1;
i_m = 1/beta*(1+pi_m)-1;
pi_eff = (1+pi_m)/(1+i_reserves)-1;
theta = parameters_US_nf(2);
sigma = parameters_US_nf(3);
B = parameters_US_nf(4);
mu_d = 0.00;
cost = 0.0202;
%% Calibrate eta
Commercial_Loan = readtimetable("Data_Calibration_US.xlsx",Sheet="Commercial Loan Rate");
...
A = fzero(@(x)calibration_func_v3(x,otherpar,psiz_grid),[0.1,2]);
[drate,nb,eq_final,d2c]= calibration_func_v3(A,otherpar,psiz_grid);
%% Save Parameters
parameters_US_final_nf=[sigma,B,nb,alpha1,alpha2,alpha3,beta,cost,eta,A,theta];
save parameters_US_final_nf.mat parameters_US_final_nf Data_TT